...
文章详情页保留搜索表单；微主页是否显示仍由主题设置控制。桌面端文章详情页的侧栏按以下顺序显示：
...
文章正文中存在真实的 `h1`～`h6` 标题时，主题会根据标题层级生成目录。目录支持多级缩进、各级目录默认展示、每个非一级目录项的层级线、当前章节高亮以及目录区域内部滚动。点击目录后，标题会在固定导航栏下方约 80px 的位置对齐。

文章没有有效的 `h1`～`h6` 标题时，不输出空的目录卡片，并恢复显示“最新回复”“标签”“归档”和“其他”侧栏卡片，避免文章详情页右侧出现空白。非文章详情页继续使用完整侧栏。
...
| `page.php` | 独立页面 |
| `post.php` | 文章详情内容和文章布局 |
| `copyright.php` | 文章版权声明公共模板 |
| `sidebar.php` | 搜索、微主页、文章目录和侧栏面板 |
...
修改 `jquery.headindex.js` 后，还需要将更新后的 HeadIndex 代码重新合并到 `js/merge.min.js`；运行时不单独加载 `jquery.headindex.js`。修改 `customs.css` 或合并脚本后，应同时更新 `functions.php` 中对应资源的版本号。
...
主题版本通过 Git Tag 管理，以下记录按照仓库实际维护日期整理。

### 2026-09-12 

#### v3.0.5

- 修复移动端导航展开时主题按钮跟随菜单移动的问题；
- 修复移动端汉堡菜单三条横线不是纯白色的问题。
...
#### v3.0.3
- 文章详情页采用搜索框、微主页、最新文章和文章目录的专用侧栏；无有效标题时隐藏空目录并恢复其他侧栏卡片；
- 目录固定在右侧栏，支持多级层级线、当前章节高亮、目录内部滚动和 80px 跳转偏移，移动端由 CSS 隐藏；
- 增加 `1600～2249px` 的 1500px 中宽屏档位，移除旧的目录左右切换和定位监听，改善响应式过渡与目录跳转定位。
